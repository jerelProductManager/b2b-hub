...
const USERS = [
  { id: "ariel", name: "Alex Rivera",  role: "B2B Manager",       avatar: "AR", canDirectPublish: true  },
  { id: "jerel", name: "Jerel Smith",    role: "Senior PM",          avatar: "JS", canDirectPublish: false },
  { id: "josh",  name: "Jordan Blake", role: "Project Management", avatar: "JB", canDirectPublish: false },
];
...
const INITIAL_CONFIG = {
  ...
  hero: {
    ...
    badge:      "Trusted by 150,000+ companies!",
  ...
  segmentPages: {
    ...
    k12: {
      ...
      featuredSignupCta:"Sign Up", featuredLearnMoreCta:"Learn More",
      otherContractIds:["tips","buyboard","equalis"], otherContractsTitle:"Other Contracts", phoneNumber:"212.239.7503",
      purchaseVolumeOptions:["Under $10,000","$10,000\u2013$50,000","$50,000\u2013$250,000","$250,000\u2013$1M","Over $1M"],
      heroImageUrl:"", signupUrl:"https://www.bhphotovideo.com/find/shared/b2bHub/signup.jsp",
    },
    federal: {
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 1000 }}>
      ...
      <div style={{ position: "absolute", top: 0, right: 0, bottom: 0, background: T.white, width: 480, maxWidth: "100vw", overflowY: "auto", boxShadow: "-8px 0 32px rgba(0,0,0,0.28)", animation: "slideInRight .25s ease", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ padding: 24 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <FloatSelect
                ...
                options={(config?.segmentPages?.[segmentId]?.purchaseVolumeOptions ?? PURCHASE_VOLS).map(v => ({ value: v, label: v }))}
              />
            </div>
        ...
        {/*
          ══ CONTACT DRAWER ══
          ─────────────────────────────────────────────────────────────────────
          BACKEND REQUIREMENT — DENIAL CONTACT PERSISTENCE (Alex session, May 2026)
          ...
          <div style={{ background: T.white, borderRadius: 12, border: `1px solid ${T.gray2}`, marginBottom: 32, overflow: "hidden" }}>
            ...
            <div style={{ borderTop: `1px solid ${T.gray2}`, padding: "28px 32px" }}>
              {selectedChannel === null ? (
                ...
                const sp      = c.segmentPages[ch.segId];
                const primaryCt = c.contracts.find(ct => ct.id === (sp?.featuredContractId || ch.primaryCtId)) || null;
                const segCts  = sp?.otherContractIds?.length
                  ? sp.otherContractIds.map(id => c.contracts.find(x => x.id === id)).filter(ct => ct?.active)
                  : c.contracts.filter(ct => ct.active && (ct.segments || []).includes(ch.segId));
                const allCts  = segCts.length > 0 ? segCts : c.contracts.filter(ct => ct.active);
                const Checks = ({ items }) => (
                ...
                return (
                  <div style={{ animation:"fadeUp .2s ease" }}>
                    <h2 style={{ fontFamily:"Montserrat,sans-serif", fontWeight:700, fontSize:22, color:T.gray6, marginBottom:16 }}>
                      {sp?.featuredTitle || ch.title}{ch.subtitle && <span style={{ fontWeight:400 }}> {ch.subtitle}</span>}
                    </h2>
                    {(sp?.intro || sp?.subIntro)
                      ? [sp?.intro, sp?.subIntro].filter(Boolean).map((p,i) => <p key={i} style={{ fontSize:14, color:T.gray6, marginBottom:12, lineHeight:1.75 }}>{p}</p>)
                    ...
                    {ch.hasContracts ? (
                      ...
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:32, marginTop:20, paddingTop:20, borderTop:`1px solid ${T.gray2}` }}>
                        <div>
                          ...
                          <Checks items={(sp?.featuredBenefits?.length ? sp.featuredBenefits : ch.benefits)} />
                        ...
                        <div style={{ borderLeft:`1px solid ${T.gray2}`, paddingLeft:32 }}>
                          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
                            <span style={{ fontSize:15, fontWeight:600, color:T.gray6 }}>{sp?.otherContractsTitle || "Other Contracts"}</span>
                            <div style={{ position:"relative" }}>
                          ...
                          <div style={{ display:"flex", flexDirection:"column", gap:2 }}>
                            {allCts.map(ct => {
                              const resolved = channelState ? resolvePortalMapping(ct, channelState, ch.segId) : null;
                              return (
                                <button key={ct.id} onClick={() => onGoContract(ct.id)} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 8px", border:"none", borderBottom:`1px solid ${T.gray1}`, background:"transparent", cursor:"pointer", textAlign:"left", width:"100%" }}>
                                  ...
                                  <div style={{ flex:1 }}>
                                    ...
                                    {resolved && resolved.portalId && (
                                      <div style={{ fontSize:11, color:T.bond, fontWeight:700 }}>🔗 Portal [{resolved.portalId}] — {resolved.label}</div>
                                    )}
                                  </div>
                                  <span style={{ color:T.gray4, fontSize:16 }}>›</span>
                                </button>
                              );
                            })}
                          </div>
                          <p style={{ fontSize:12, color:T.gray5, marginTop:14 }}>Can't find the contract you need? Call us at <strong>{sp?.phoneNumber || "212.239.7503"}</strong></p>
                        </div>
                      </div>
                    ) : ch.hasEprocurement ? (
                  ...
                  return (
                    <div style={{ marginBottom: 16 }}>
                      ...
                      {resolvedFeatured && resolvedFeatured.portalId && (
                        <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: T.bondLight, borderRadius: 4, padding: "4px 10px", fontSize: 11, fontWeight: 700, color: T.bond, marginLeft: 10 }}>
                          🔗 Portal [{resolvedFeatured.portalId}] — {resolvedFeatured.label}
                        </div>
                      )}
                    </div>
                  );
    ...
    <div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 24px" }}>
        ...
        {/* "Available Portals by State & Segment" was removed from the customer-facing
            contract page (Alex session, May 2026). Admins can still see and manage
          ...
          <div style={{ padding: "32px 24px" }}>
            {pageId === "studio" ? (
              <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                ...
                <div>
                  <h1 style={{ fontFamily: "Montserrat,sans-serif", fontSize: 26, fontWeight: 800, color: T.white }}>{headline}</h1>
                  <span style={{ color: "#ccc", fontSize: 14, fontStyle: "italic" }}>{cp.tagline || "Tech Solutions for Media Professionals"}</span>
                </div>
              </div>
            ) : (
              <h1 style={{ fontFamily: "Montserrat,sans-serif", fontSize: 28, fontWeight: 800, color: T.white }}>{headline}</h1>
            )}
          </div>
        </div>
      </HeroBanner>
        ...
        {pageId === "contact" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 32, marginBottom: 40 }}>
              <div>
                <p style={{ fontSize: 14, color: T.gray6, lineHeight: 1.7, marginBottom: 10 }}>{cp.intro || "At B&H B2B we still believe in the personal touch. Each B2B customer gets assigned a personal Account Team, ensuring that a \"live\" agent will be available to assist you with your RFQs, purchasing and customer service needs from A to Z."}</p>
                <p style={{ fontSize: 14, color: T.gray5, lineHeight: 1.7 }}>{cp.body || "We stock the widest selection of security, IT, multimedia, optics and A/V presentation gear anywhere… which makes us a true one-stop shop for your organization."}</p>
              </div>
...
function SegmentPortalRoutingEditor({ segId, config, onChange }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      {relContracts.map(({ ct, i: contractIndex }) => {
        const allMappings = ct.portalMappings || [];
        const relevantMappings = allMappings.filter(m => m.segment === segId || m.segment === "");

        const addMapping = () => {
          onChange(d => {
            if (!d.contracts[contractIndex].portalMappings) d.contracts[contractIndex].portalMappings = [];
            d.contracts[contractIndex].portalMappings.push({ id: `mapping_${Date.now()}`, state: "", segment: segId, portalId: "", label: "" });
          });
        };
        const removeMapping = (mappingId) => {
          onChange(d => { d.contracts[contractIndex].portalMappings = d.contracts[contractIndex].portalMappings.filter(m => m.id !== mappingId); });
        };
        const updateMapping = (mappingId, field, value) => {
          onChange(d => { const m = d.contracts[contractIndex].portalMappings.find(x => x.id === mappingId); if (m) m[field] = value; });
        };

        return (
          <div key={ct.id} style={{ border: `1.5px solid ${T.gray2}`, borderRadius: 10, overflow: "hidden" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 14px", background: T.gray1, borderBottom: `1px solid ${T.gray2}` }}>
              <ContractLogo contract={ct} height={16} maxWidth={52} />
              <span style={{ fontWeight: 700, fontSize: 13, color: T.gray6 }}>{ct.name}</span>
              <span style={{ marginLeft: "auto", fontSize: 11, fontWeight: 700, color: relevantMappings.length > 0 ? T.green : T.gray4, background: relevantMappings.length > 0 ? T.greenLight : T.gray2, padding: "2px 8px", borderRadius: 10 }}>
                {relevantMappings.length} mapping{relevantMappings.length !== 1 ? "s" : ""}
              </span>
            </div>
            <div style={{ padding: "12px 14px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "110px 90px 1fr 32px", gap: 6, padding: "0 4px", marginBottom: 6 }}>
                {["State", "Portal ID", "Label / Display Name", ""].map(h => (
                  <div key={h} style={{ fontSize: 10, fontWeight: 700, color: T.gray4, textTransform: "uppercase", letterSpacing: .4 }}>{h}</div>
                ))}
              </div>
              {relevantMappings.length === 0 && (
                <div style={{ fontSize: 12, color: T.gray4, fontStyle: "italic", padding: "8px 4px", marginBottom: 6 }}>
                  No mappings for this segment. Add a row — leave State as "All States" to set the national default.
                </div>
              )}
              {relevantMappings.map((m, rowIdx) => {
                const isCrossSegment = m.segment === "";
                return (
                  <div key={m.id} style={{ display: "grid", gridTemplateColumns: "110px 90px 1fr 32px", gap: 6, alignItems: "center", marginBottom: 6, padding: "6px 4px", background: rowIdx % 2 === 0 ? T.white : "#fafafa", border: `1px solid ${isCrossSegment ? T.orange : T.gray2}`, borderRadius: 6 }}>
                    <div>
                      <select value={m.state} onChange={e => updateMapping(m.id, "state", e.target.value)} style={{ padding: "5px 5px", border: `1px solid ${T.gray3}`, borderRadius: 5, fontSize: 12, background: T.white, width: "100%" }}>
                        <option value="">All States</option>
                        {STATES.map(s => <option key={s} value={s}>{s}</option>)}
                      </select>
                      {isCrossSegment && <div style={{ fontSize: 9, color: T.orange, fontWeight: 700, marginTop: 2, letterSpacing: .3 }}>ALL SEGMENTS</div>}
                    </div>
                    <input value={m.portalId} onChange={e => updateMapping(m.id, "portalId", e.target.value)} placeholder="e.g. 944" style={{ padding: "6px 8px", border: `1px solid ${T.gray3}`, borderRadius: 5, fontSize: 12, fontFamily: "monospace", background: T.white, width: "100%" }} />
                    <input value={m.label} onChange={e => updateMapping(m.id, "label", e.target.value)} placeholder="e.g. E&I Virginia Higher Ed" style={{ padding: "6px 8px", border: `1px solid ${T.gray3}`, borderRadius: 5, fontSize: 12, background: T.white, width: "100%" }} />
                    <button onClick={() => removeMapping(m.id)} style={{ background: T.scarletLight, border: "none", borderRadius: 5, color: T.scarlet, fontWeight: 700, fontSize: 13, cursor: "pointer", width: 28, height: 28, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>✕</button>
                  </div>
                );
              })}
              <button onClick={addMapping} style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4, background: T.bondLight, color: T.bond, border: `1px dashed ${T.bond}`, borderRadius: 6, padding: "6px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
                + Add State Override
              </button>
            </div>
          </div>
  ...
  if (section.startsWith("seg_")) {
    ...
    return (
      <div>
        ...
        <div style={{ marginBottom: 18 }}>
          <label style={{ fontSize: 12, fontWeight: 700, color: T.gray5, textTransform: "uppercase", letterSpacing: .4, display: "block", marginBottom: 6 }}>Annual Purchase Volume Options (one per line)</label>
          <div style={{ fontSize: 11, color: T.gray4, marginBottom: 6 }}>Each line becomes a selectable option in the sign-up form for this segment. Overrides the global defaults.</div>
          <textarea
            value={(sp.purchaseVolumeOptions || []).join("\n")}
            onChange={e => onChange(d => { d.segmentPages[segId].purchaseVolumeOptions = e.target.value.split("\n").filter(Boolean); })}
            rows={6}
        ...
        <div style={{ marginBottom: 18, borderTop: `2px solid ${T.gray2}`, paddingTop: 20 }}>
          <label style={{ fontSize: 12, fontWeight: 700, color: T.gray5, textTransform: "uppercase", letterSpacing: .4, display: "block", marginBottom: 4 }}>
            🔗 State × Contract Portal Routing
          </label>
          <div style={{ fontSize: 11, color: T.gray4, marginBottom: 12 }}>
            For each contract assigned to this segment, control which portal ID customers are routed to by state. "All States" sets the national default. Exact state match takes priority. Changes here are also reflected in the Contracts section.
          </div>
          <SegmentPortalRoutingEditor segId={segId} config={config} onChange={onChange} />
        </div>
...
function ApprovalsView({ pendingChanges, liveConfig, currentUser, onApprove, onReject, onPreview }) {
  ...
  const isAlex = currentUser.canDirectPublish;
  return (
    <div style={{ maxWidth: 920, margin: "0 auto", padding: "28px 24px" }}>
      <div style={{ marginBottom: 24 }}>
        <h2 style={{ fontFamily: "Montserrat,sans-serif", fontSize: 22, fontWeight: 800, marginBottom: 6 }}>{isAlex ? "Approval Queue" : "Pending Submissions"}</h2>
        <p style={{ fontSize: 14, color: T.gray5 }}>{isAlex ? "As Alex Rivera, you can approve or reject proposed changes. Approved changes go live immediately." : "Changes you submit go to Alex Rivera for review before going live."}</p>
      </div>
      <div style={{ background: T.bondLight, border: `1px solid ${T.bond}`, borderRadius: 10, padding: "14px 18px", marginBottom: 24, display: "flex", gap: 16 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 13, color: "#444", lineHeight: 1.6 }}>
            <strong>Alex Rivera</strong> can publish directly — no approval required.<br />
            <strong>All other admins</strong> submit changes that Alex must approve before going live.<br />
      ...
      {pending.length > 0 ? (
        <div style={{ marginBottom: 32 }}>
          ...
          {pending.map(c => <ChangeCard key={c.id} change={c} isAlex={isAlex} onApprove={onApprove} onReject={onReject} onPreview={onPreview} liveConfig={liveConfig} />)}
        </div>
      ) : (
      ...
      {resolved.length > 0 && (
        <div>
          ...
          {resolved.map(c => <ChangeCard key={c.id} change={c} isAlex={isAlex} onApprove={onApprove} onReject={onReject} onPreview={onPreview} liveConfig={liveConfig} resolved />)}
        </div>
      )}
    </div>
  );
...
function ChangeCard({ change, isAlex, onApprove, onReject, onPreview, resolved }) {
